...
1. integer: 4 байта
2. tinyint: 1 байт
3. varchar(n): от 1 до n байт, плюс 1-2 байта для хранения длины (в среднем возьмем 255 байт, исключение - текст отзыва, био профиля, описание объекта)
4. timestamp: 8 байт
5. date: 3 байта
...
| ----------------- | ------------------------------------------------------ | --------------------- | ----------------------- |
| users             | Информация о пользователях сервиса                     | 1 КБ                  | 1 * 80 млн = 80 ГБ      |
| reviews           | Отзывы пользователей об объектах                       | 1,5 КБ                | 1,5 * 1 млрд = 1500 ГБ  |
| reviews_responses | Ответы представителей объектов на отзывы пользователей | 1 КБ                  | 1 * 100 млн = 100 ГБ    |
| reviews_photos    | Фотографии, прикрепленные к отзывам                    | 0,25 КБ               | 0,25 * 160 млн = 40 ГБ  |
| objects           | Данные об объектах                                     | 2,5 КБ                | 2,5 * 8 млн = 20 ГБ     |
| objects_photos    | Фотографии объектов                                    | 0,25 КБ               | 0,25 * 80 млн = 20 ГБ   |
| trips             | Созданные пользователями маршруты (списки объектов)    | 0,25 КБ               | 0,25 * 80 млн = 20 ГБ   |
| trips_objects     | Связь между маршрутами и объектами                     | 0,008 КБ              | 0,008 * 160 млн = 12 ГБ |
...
- если запись, на которую ссылается внешний ключ, удаляется, необходимо удалить или обновить все связанные записи, чтобы избежать "висячих" ссылок
