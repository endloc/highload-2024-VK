...
Итак, сервис пользуется наибольшей популярностью в Европе и в США. Расположим в этих регионах по 2 ДЦ, так как, хоть и большее число пользователей из США, отзывы публикуются чаще всего из Европы (активная аудитория).
...
- Таблица **trips**
  - `user_id` для поиска маршрутов одного пользователя
- Таблица **objects**
...
Как указано в разделе [3. Глобальная балансировка нагрузки](#3-глобальная-балансировка-нагрузки), имеем 6 ДЦ. Посчитаем нагрузку и количество требуемых ресурсов CPU и RAM для 1 ДЦ. Сервисы написаны на языке Go. Условно разделим сервисы по характеру сложности бизнес-логики:
...
| -------------- | ---------------- | -------------------- | ----------------- | ----------------- | ----------------- |
| Auth           | 17               | 0,00015              | легкая            | 1                 | 0,1               |
| User           | 466              | 0,03829              | средняя           | 5                 | 2,5               |
| Search         | 58               | 0,02340              | средняя           | 1                 | 0,5               |
| Object         | 174              | 0,02910              | тяжелая           | 18                | 1,8               |
| Review         | 665              | 0,02712              | средняя           | 7                 | 3,5               |
| Recommendation | 27               | 0,00214              | средняя           | 1                 | 0,5               |
| Trip           | 58               | 0,02340              | средняя           | 1                 | 0,5               |
| File           | 248              | 2,00000              | средняя           | 3                 | 1,5               |
| **Итого**      | **1711**         | **2,1 + 20% = 2,52** | -                 | **37 + 20% = 42** | **11 + 20% = 13** |


Количество ядер и память взяты с запасом в 20%: дальнейший рост + случай, если 1 ДЦ на время выйдет из строя.

Модель развертывания: Kubernetes (автоматизированное развертывание, управление и масштабирование приложений, упакованных в контейнеры; надежность; изоляция контейнеров). В качестве хостинга выберем собственное железо:
- Обеспечивает полный контроль над инфраструктурой.
- Позволяет избежать постоянных затрат на облачные услуги, что может значительно снизить общие расходы при предсказуемых нагрузках.
- Обеспечивает высокую производительность и безопасность данных.

Общее количество железа для сервисов Kubernetes (на все ДЦ):

| Название  | Конфигурация                                                                                                                     | CPU cores | RAM | Количество | Аренда: **месяц** / **3 года** | Покупка                     |
| --------- | -------------------------------------------------------------------------------------------------------------------------------- | --------- | --- | ---------- | ------------------------------ | --------------------------- |
| kube node | DELL PowerEdge R440 Intel Xeon Silver R440-5201-10<br>1x4116 1x16Gb 2RRD x4 3.5" RW H730p LP iD9En 1G 2P 1x550W 3Y PNBD -5201-10 | 12        | 16  | 4 * 6 = 24 | $960 ($40 за шт) / $34,560     | $50,400 ($2,100 за шт[^17]) |
...
[^17]: [DELL PowerEdge R440 Intel Xeon Silver R440-5201-10](https://shop.nav-it.ru/catalogue/stoechnye_servery_rack/server_dell_poweredge_r440_1x4116_1x16gb_2rrd_x4_3_5_rw_h730p_lp_id9en_1g_2p_1x550w_3y_pnbd_r440_520_1/)
