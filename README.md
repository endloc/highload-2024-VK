...
| ----------------------------------------------------------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Асинхронный пересчет рейтингов объектов через очередь сообщений (Kafka) | Рейтинги объектов                       | С некоторой периодичностью в Kafka отправляются сообытия (например, события с ID всех элементов, для которых поставили оценку в течение последних пяти минут); после обработки событий данные о рейтинге обновляются в кэше и в БД | Не перегружаем систему, обеспечивая при этом достаточную актуальность данных                                                                        |
| LRU (Least Recently Used)                                               | Управление кэшом, связанным с объектами | Удаляет из кэша самые неиспользуемые данные. В отличие от модели FIFO, LRU смотрит не на время добавления, а на то, когда информацию использовали в последний раз.                                                                 | Поддерживаем в кэше наиболее актуальные данные, что существенно ускоряет доступ к часто запрашиваемой информации и снижает нагрузку на базу данных. |
