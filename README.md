...
| --------------------------------------- | ------------------------------------------------------ | -------------- |
| **users**               (MongoDB)       | Пользователи сервиса                                   | По user_id     |
| **roles**               (MongoDB)       | Роли, существующие для пользователей системы           | -              |
| **reviews**             (MongoDB)       | Отзывы пользователей об объектах                       | По object_id   |
| **reviews_responses**   (MongoDB)       | Ответы представителей объектов на отзывы пользователей | -              |
| **reviews_photos**      (MongoDB)       | Фотографии, прикрепленные к отзывам                    | По review_id   |
| **objects**             (MongoDB)       | Данные об объектах                                     | По category_id |
| **categories**          (MongoDB)       | Категории объектов                                     | -              |
| **objects_photos**      (MongoDB)       | Фотографии объектов                                    | По object_id   |
| **trips**               (MongoDB)       | Созданные пользователями маршруты (списки объектов)    | -              |
| **trips_objects**       (MongoDB)       | Связь между маршрутами и объектами                     | По trip_id     |
| **locations**           (MongoDB)       | Адреса объектов                                        | -              |
| **countries**           (MongoDB)       | Предварительно заполненная таблица со списком стран    | -              |
| **recommendations**     (MongoDB)       | Рекомендации для пользователей                         | -              |
| **sessions**            (Redis)         | Информация о сессиях пользователей                     | -              |
| **object_search_cache** (Redis)         | Кэшированные результаты поисковых запросов             | -              |
...
- **objects:** 
  - `location_id` для поиска объектов по местоположению
  - `rating` для фильтрации объектов по рейтингу
- **trips:**
